...
void initMainScreen_internal()
{
    ...
    printf("[initMainScreen_internal] Initialisation de l'écran principal.\n");
                          ...
                          lv_color_t color)
{
    ...
    printf("[displayText_internal] Affichage du texte.\n");
    printf("  Texte : '%s'\n", text);
    printf("  Position : (%d, %d)\n", x, y);
    printf("  Taille de police : %d\n", fontSize);
    printf("  Couleur : 0x%04X\n", color.full);

    lv_obj_t* label = lv_label_create(main_screen);
    if (label == NULL)
    {
        printf("[displayText_internal] Échec de la création du label.\n");
        return;
    }
    ...
    {
        printf(
            "[displayText_internal] Police '%s' de taille %d non trouvée. Utilisation de la police par défaut.\n",
            fontName, fontSize);
    }
...
void displayIcon_internal(const char* iconName, int16_t x, int16_t y, float scale, lv_color_t color)
{
    ...
    printf("[displayIcon_internal] Affichage de l'icône.\n");
    printf("  Icône: '%s'\n", iconName);
    printf("  Position: (%d, %d)\n", x, y);
    printf("  Échelle: %.2f\n", scale);
    printf("  Couleur: 0x%04X\n", color.full);
    ...
    if (icon_dsc)
    {
        ...
        if (scale > 0.0f)
        {
            ...
            printf("  Facteur de zoom calculé: %u\n", zoom_factor);
            lv_img_set_zoom(img_obj, zoom_factor);
        }
        else
        {
            printf("  Échelle invalide: %.2f\n", scale);
        }
    ...
    {
        printf("[displayIcon_internal] Impossible d'afficher l'icône '%s'.\n", iconName);
    }
}
...
void clearScreen_internal(lv_color_t color)
{
    ...
    printf("[clearScreen_internal] Nettoyage de l'écran avec la couleur 0x%04X.\n", color.full);
...
void displayRectangle_internal(int16_t x, int16_t y, uint16_t width, uint16_t height, lv_color_t color)
{
    ...
    printf("[displayRectangle_internal] Affichage d'un rectangle.\n");
    printf("  Position : (%d, %d)\n", x, y);
    printf("  Taille : %d x %d\n", width, height);
    printf("  Couleur : 0x%04X\n", color.full);
    ...
    if (rect == nullptr)
    {
        printf("[displayRectangle_internal] Échec de la création de l'objet rectangle.\n");
        return;
    }
...
{
    printf("[setScreenBrightness_internal] Réglage de la luminosité de l'écran à %d.\n", brightness);
...
void displaySpinner_internal(int16_t x, int16_t y, float scale, lv_color_t color, float spins_per_second)
{
    ...
    printf("[displaySpinner_internal] Affichage d'un spinner.\n");
    printf("  Position : (%d, %d)\n", x, y);
    printf("  Échelle : %.2f\n", scale);
    printf("  Couleur : 0x%04X\n", color.full);
    printf("  Vitesse : %.2f tours/s\n", spins_per_second);
    ...
    if (spinner == NULL)
    {
        printf("[displaySpinner_internal] Échec de la création du spinner.\n");
        return;
    }
